...
# -------------------- 30 base templates --------------------
def make_basic_30(g: str, m: str) -> List[Dict]:
    ...
    out.append(q("List all genes.", "MATCH (g:Gene) RETURN g LIMIT 100", "list_genes"))
    out.append(q("List all MeSH entitys.", "MATCH (m:MeSH) RETURN m LIMIT 100", "list_mesh"))
    out.append(q("List literature nodes.", "MATCH (l:Literature) RETURN l LIMIT 100", "list_lit"))
    ...
    out.append(q(
        f"Top 10 MeSH for {g} by Strength.",
        f"MATCH (:Gene {{entity:'{g}'}})-[r:CO_OCCURS]-(m:MeSH) RETURN m, r.Strength AS strength ORDER BY strength DESC LIMIT 10",
        "top_by_strength"
    ))
    ...
    out.append(q(
        ...
        f"MATCH (:Gene {{entity:'{g}'}})-[:CO_OCCURS]-(m:MeSH) RETURN DISTINCT m SKIP 20 LIMIT 20",
    ...
    out.append(q(
        f"Order {g}-linked MeSH by Strength then paper count.",
        f"MATCH (:Gene {{entity:'{g}'}})-[r:CO_OCCURS]-(m:MeSH) OPTIONAL MATCH (m)<-[:HAS_SOURCE]-(l:Literature) WITH m, r, count(DISTINCT l) AS nPapers RETURN m, r.Strength AS strength, nPapers ORDER BY strength DESC, nPapers DESC",
        "multi_sort"
    ))
...
def main():
    ...
    per_draw = 30 + 12 + 1  # 30 basic + 12 extra + 1 thematic-year
    ...
    for _ in range(draws_needed):
        g = choose(genes)
        m = choose(meshs)
        batch = make_basic_30(g, m) + make_extra(g, m, genes, meshs)
